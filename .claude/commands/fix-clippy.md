...
1. `cargo clippy --fix --workspace --all-targets --allow-dirty` を実行し、自動修正可能な警告を先に修正する
2. `cargo clippy --workspace --all-targets` を実行し、残った警告を確認する
3. 残った警告があれば、コードを手動で修正する（各警告の内容に応じて適切に対処）
4. `cargo clippy --workspace --all-targets` で警告ゼロを確認する
