...
- 最大 3 手先読み AI（ヒューリスティック評価 / CNN 評価）
...
├─────────────────────────────────┤
│  NN (puyo-nn)                   │  CNN 価値ネットワーク・エンコーディング
├─────────────────────────────────┤
...
1. 現在のぷよ組と次のぷよ組（NN モード時はさらに次の次）について、全配置パターンを列挙
2. 各配置後の盤面を評価関数でスコアリング
   - **ヒューリスティック**: 連鎖スコア、連鎖長、高さペナルティ、高さ分散、連結度、潜在連鎖、中央寄り（7 項目 + ゲームオーバー判定）
   - **CNN**: ニューラルネットワークによる盤面評価（10ch エンコーディング）
