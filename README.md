...
2つの Evaluator を切り替え可能:

- **SimulationEvaluator**（ヒューリスティック）
  1. 現在のぷよ組・次のぷよ組について、最大2手先の全配置パターンをBFS順で列挙
  2. 各配置後の盤面に仮想ぷよ（同色2個）を落として連鎖スコアの期待値を推定
  3. 全深度を通じて最高評価を得た1手目の配置を選択

- **NnEvaluator**（ニューラルネットワーク）
  - **Policy-only モード**（WASM用）: Dual Head CNN（PuyoNet）が盤面+ツモから直接配置確率を出力。探索なし、1回の推論で手を選択
  - **MCTS モード**（学習用）: Policy Head + Value Head を使ったモンテカルロ木探索で手を選択
