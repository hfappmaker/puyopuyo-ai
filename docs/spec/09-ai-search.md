...
シミュレーション中に10連鎖以上（`chain_count >= 10`）が発生した場合、評価関数のスコアに関わらず即座にその配置を選択する（早期リターン）。スコアは `f64::INFINITY` として返される。

- **search_depth1**: 各配置のシミュレーション後に連鎖数をチェックし、10連鎖以上なら即リターン
- **search_depth2**: 1手目・2手目の両方をチェック。いずれかで10連鎖以上が発生した場合、その1手目の配置を即リターン
